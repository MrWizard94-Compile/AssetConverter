...
public class AsyncLocator {
    ...
    public static void setupExecutorService() {
        ...
        int threads = ServerConfig.SERVER.locating_threads.get();
        ArsAdditions.LOGGER.info("Starting locating executor service with thread pool size of {}", threads);
        LOCATING_EXECUTOR_SERVICE = Executors.newFixedThreadPool(
                threads,
                new ThreadFactory() {
                    private static final AtomicInteger poolNum = new AtomicInteger(1);
                    private final AtomicInteger threadNum = new AtomicInteger(1);
                    private final String namePrefix = ArsAdditions.MODID + "-" + poolNum.getAndIncrement() + "-thread-";
    ...
    public static LocateTask<BlockPos> locate(
        ...
        logCreateTask(structureTag.toString(), level, pos, searchRadius);
    ...
    public static LocateTask<Pair<BlockPos, Holder<Structure>>> locate(
        ...
        logCreateTask(structureSet.toString(), level, pos, searchRadius);
    ...
    private static void doLocateLevel(
        ...
        logLocateAttempt(structureTag.toString(), level, pos, searchRadius);
        ...
        if (foundPos == null)
            logNotFound(structureTag.toString(), time);
        else
            logFound(structureTag.toString(), foundPos, time);
    ...
    private static void doLocateChunkGenerator(
        ...
        logLocateAttempt(structureSet.toString(), level, pos, searchRadius);
        long start = System.nanoTime();
        ...
        String time = NumberFormat.getNumberInstance().format(TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - start));
        if (foundPair == null)
            logNotFound(structureSet.toString(), time);
        else
            logFound(foundPair.getSecond().value().getClass().getSimpleName(), foundPair.getFirst(), time);
    ...
    private static void logCreateTask(String structure, ServerLevel level, BlockPos pos, int searchRadius) {
        ArsAdditions.LOGGER.debug(CREATE_TASK_MESSAGE, structure, level, pos, searchRadius);
    }

    private static void logLocateAttempt(String structure, ServerLevel level, BlockPos pos, int searchRadius) {
        ArsAdditions.LOGGER.info(ATTEMPT_LOCATE_MESSAGE, structure, level, pos, searchRadius);
    }

    private static void logNotFound(String structure, String time) {
        ArsAdditions.LOGGER.info(NOT_FOUND_MESSAGE, structure, time);
    }

    private static void logFound(String structure, BlockPos pos, String time) {
        ArsAdditions.LOGGER.info(FOUND_MESSAGE, structure, pos.toString(), time);
    }
